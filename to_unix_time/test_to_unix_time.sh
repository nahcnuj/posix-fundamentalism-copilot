...
assert_eq() {
    input=$1
    expected=$2
    actual=$(printf '%s\n' "$input" | "$SHELL" "$SCRIPT_DIR/to_unix_time.sh")
...
assert_eq_with_tz() {
    ...
    actual=$(printf '%s\n' "$input" | PATH="$fake_dir:$PATH" "$SHELL" "$SCRIPT_DIR/to_unix_time.sh")
    rc=$?
    rm -rf "$fake_dir"
...
assert_rejects_invalid_tz() {
    ...
    printf '#!/bin/sh\necho BADTZ\n' > "$fake_dir/date"
    chmod +x "$fake_dir/date"
    printf '19700101000000\n' | PATH="$fake_dir:$PATH" "$SHELL" "$SCRIPT_DIR/to_unix_time.sh" >/dev/null 2>&1
    rc=$?
    rm -rf "$fake_dir"
    if [ "$rc" -ne 0 ]; then
...
# covering the PATH-based invocation where script_path is resolved via command -v.
assert_eq_noslash() {
    input=$1
    expected=$2
    abs_script_dir=$(cd "$SCRIPT_DIR" && pwd) || return 1
    fake_dir=$(mktemp -d "${TMPDIR:-/tmp}/test_to_unix_time_noslash.XXXXXX") || return 1
    actual=$(
        cd "$fake_dir" && \
        printf '%s\n' "$input" | PATH="$abs_script_dir:$PATH" "$SHELL" to_unix_time.sh
    )
    rc=$?
    rm -rf "$fake_dir"
    if [ "$rc" -eq 0 ] && [ "$actual" = "$expected" ]; then
...
assert_eq_noslash "$(unix_to_local 0)" "0"
