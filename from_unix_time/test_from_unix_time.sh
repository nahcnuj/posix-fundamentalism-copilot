...
assert_bare_name_via_path() {
    ...
    printf '#!/bin/sh\necho +0000\n' > "$fake_dir/date"
    chmod +x "$fake_dir/date"
    actual=$(printf '0\n' | PATH="$SCRIPT_DIR:$fake_dir:$PATH" "$SHELL" from_unix_time.sh)
    rc=$?
    rm -rf "$fake_dir"
    if [ "$rc" -eq 0 ] && [ "$actual" = "19700101000000" ]; then
        printf 'PASS: from_unix_time.sh (bare name via PATH) -> %s\n' "$actual"
        PASS=$((PASS + 1))
    else
        printf 'FAIL: from_unix_time.sh (bare name via PATH) -> expected 19700101000000, got %s (exit %d)\n' "$actual" "$rc"
        FAIL=$((FAIL + 1))
    fi
}
assert_bare_name_via_path
